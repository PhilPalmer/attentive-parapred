...
def kfold_cv_eval(dataset, output_file="crossval-data.p",
    ...
    cdrs, lbls, masks, lengths, ag, ag_masks, ag_lengths, dist_mat, delta_gs = \
        ...
        dataset["ag"], dataset["ag_masks"], dataset["ag_lengths"], dataset["dist_mat"], torch.FloatTensor(dataset["delta_gs"])

    print("cdrs", cdrs.shape)
    print("ag", ag.shape)
    print("delta_gs", delta_gs.shape)
        ...
        if code == 7:
            probs_test, lbls_test = \
                xself_run(cdrs_train, lbls_train, mask_train, lengths_train,
                            ...
                            cdrs_test, lbls_test, mask_test, lengths_test,
                            ag_test, ag_masks_test, ag_lengths_test, dist_mat_test, delta_gs_test)
