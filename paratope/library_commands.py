...
def get_predictor(id_model="FP"):
    ...
    from atrous import DilatedConv
    if _model is None:
        if id_model == "L":
            _model = RNNModel()
        ...
        if id_model == "D":
            _model = DilatedConv()
            weights = pkg_resources.resource_filename(__name__, "cv-ab-seq/atrous_weights.pth.tar")
...
def process_single_pdb(pdb_name, model_type, ab_h_chain, ab_l_chain):
    ...
    if model_type == "AFP" or model_type == "AFPX":
        ...
        print_delta_g(model=model, visualisation_pdb=pdb_name)
    else:
        print_probabilities(model, model_type=model_type,out_file_name= pdb_name, visualisation_pdb=pdb_name)
